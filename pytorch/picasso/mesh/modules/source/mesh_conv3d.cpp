...
void facet2facetConv3dGradLauncher(int NfIn, int Cin, int Cout, const int* numInterior, const float* intrplWgts,
    ...
    torch::Tensor vt_map)       // vertex mapping from input to output vertices: concat_NvIn
{
    ...
    auto grad_coeff  = torch::zeros({NfIn,K}, input.options());

    float* gradInput_ptr  = grad_input.data_ptr<float>();
    float* gradFilter_ptr = grad_filter.data_ptr<float>();
    float* gradCoeff_ptr  = grad_coeff.data_ptr<float>();

    facet2vertexConv3dGradLauncher(NfIn, C, r, K, vtMap_ptr, nfCount_ptr, face_ptr, coeff_ptr, input_ptr,
                                   filter_ptr, gradOut_ptr, gradInput_ptr, gradFilter_ptr, gradCoeff_ptr);

    return {grad_input, grad_filter, grad_coeff};
}
    ...
    torch::Tensor num_texture)      // number of interior interpolated: concat_NfIn
{
    ...
    TORCH_CHECK(filter.size(1)==Cin && filter.size(2)==3,
                "3 filters required, or input Channel size error of the filter");
    TORCH_CHECK(bary_coeff.dim()==2 && bary_coeff.size(0)==NiK && bary_coeff.size(1)==3,
                "The shape of bary_coeff should be (NiK, 3)).");
    ...
    facet2facetConv3dLauncher(NfIn, Cin, Cout, numTexture_ptr, baryCoeff_ptr,
                              input_ptr, filter_ptr, output_ptr);
    return output;
}
    ...
    torch::Tensor num_texture)  // number of interior interpolated: concat_NfIn
{
    ...
    TORCH_CHECK(filter.size(1)==Cin && filter.size(2)==3,
                "3 filters required, or input Channel size error of the filter");
    TORCH_CHECK(bary_coeff.dim()==2 && bary_coeff.size(0)==NiK && bary_coeff.size(1)==3,
                "The shape of input vertex should be (NiK, 3)).");
    ...
    auto grad_filter = torch::zeros({Cout,Cin,3}, input.options());
    float* gradInput_ptr  = grad_input.data_ptr<float>();
    float* gradFilter_ptr = grad_filter.data_ptr<float>();

    facet2facetConv3dGradLauncher(NfIn, Cin, Cout, numTexture_ptr, baryCoeff_ptr, input_ptr,
