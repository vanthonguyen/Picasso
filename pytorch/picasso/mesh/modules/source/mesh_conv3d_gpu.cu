...
// filter:      (Cout, Cin, 3)
// output:      (NfIn, Cout)
__global__ void facet2facet_conv3d_forward(int NfIn, int Cin, int Cout, const int* numInterior,
                                           ...
                                           const float* filter, float* output)
{
    ...
    if(fcIdx<NfIn)
    {
        ...
        {
            int ch = ch_out*Cin*3 + ch_in*3;
            for(int k=kStart;k<kEnd;k++)
            {
                // get interpolation weights (w1,w2,w3) related to (v1->v2->v3) of the face
                float w1=baryCoeff[k*3], w2=baryCoeff[k*3+1], w3=baryCoeff[k*3+2];

                float weight = w1*filter[ch] + w2*filter[ch+1] + w3*filter[ch+2];
...
// gradInput:   (NiK, Cin)
__global__ void facet2facet_input_backward(int NfIn, int Cin, int Cout, const int* numInterior,
...
// input:       (NiK, Cin)
// gradOutput:  (NfIn, Cout)
// gradFilter:  (Cout, Cin, 3)
__global__ void facet2facet_filter_backward(int NfIn, int Cin, int Cout, const int* numInterior,
...
void facet2facetConv3dLauncher(int NfIn, int Cin, int Cout, const int* numInterior, const float* baryCoeff,
                               const float* input, const float* filter, float* output)
{
    int numGrid = NfIn*Cout/1024 + 1;
    facet2facet_conv3d_forward<<<numGrid,1024>>>(NfIn, Cin, Cout, numInterior, baryCoeff,
                                                 input, filter, output);

}
void facet2facetConv3dGradLauncher(int NfIn, int Cin, int Cout, const int* numInterior, const float* baryCoeff,
                                   const float* input, const float* filter, const float* gradOutput,
                                   float* gradInput, float* gradFilter)
{
    int numGrid = NfIn*Cout/1024 + 1;
    facet2facet_input_backward<<<numGrid,1024>>>(NfIn, Cin, Cout, numInterior, baryCoeff, filter,
    ...
    int maxIter = (3*Cout*Cin)/maxSharedMemSize;
    int remainder = (3*Cout*Cin)%maxSharedMemSize;
    for(int iter=0;iter<maxIter;iter++)
    {
        facet2facet_filter_backward<<<numGrid,1024,sizeof(float)*maxSharedMemSize>>>(NfIn, Cin, Cout, numInterior,
                                                                            baryCoeff, input, gradOutput, gradFilter,
                                                                            maxSharedMemSize, maxSharedMemSize*iter);
    }
    if(remainder>0) // fill the remainder
    {
        facet2facet_filter_backward<<<numGrid,1024,sizeof(float)*remainder>>>(NfIn, Cin, Cout, numInterior,
                                                                        baryCoeff, input, gradOutput, gradFilter,
...
__global__ void facet2vertex_coeff_backward(int NfIn, int C, int r, const int K, const int* vtMap,
                                    const int* nfCount, const int* face, const float* coeff, const float* input,
                                    ...
                                    float* gradInput, float* gradFilter, float* gradCoeff)
{
    ...
    facet2vertex_coeff_backward<<<numGrid,1024>>>(NfIn, C, r, K, vtMap, nfCount, face, coeff, input, filter,
                                                  gradOutput, gradCoeff);
}
