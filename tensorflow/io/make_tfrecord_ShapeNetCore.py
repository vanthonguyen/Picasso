...
def make_tfrecord_cls(model_path, cls_label, store_folder, visualize=False):
    # load the mesh
    ...
    filename = os.path.join(store_folder, '%s.tfrecord'%model_path.split('/')[-1])
    if not os.path.exists(filename):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'label_raw': _int64_feature(cls_label)}))
...
if __name__=='__main__':
    ...
    for k in tqdm(range(total_samples)):
        ...
        if dataDict[dict_keys[-1]][k]=='train':
            nV_nF = make_tfrecord_cls(modelPath, cls_label, store_folder+'/train', LOG_train, visualize=False)
            if nV_nF is not None:
                num_vertex_facet['train'].append(nV_nF)
        elif dataDict[dict_keys[-1]][k]=='val':
            nV_nF = make_tfrecord_cls(modelPath, cls_label, store_folder+'/val', LOG_val, visualize=False)
            if nV_nF is not None:
                num_vertex_facet['val'].append(nV_nF)
        elif dataDict[dict_keys[-1]][k]=='test':
            nV_nF = make_tfrecord_cls(modelPath, cls_label, store_folder+'/test', LOG_test, visualize=False)
            if nV_nF is not None:
