...
parser.add_argument('--log_dir', default='../log_s3dis_plain', help='Log dir [default: log_s3dis_plain]')
...
def parse_fn(item, is_training=True):
    ...
    rgb = tf.io.decode_raw(features['rgb_raw'], tf.float32)
    face = tf.io.decode_raw(features['face_raw'], tf.int32)
    label = tf.io.decode_raw(features['seg_label_raw'], tf.int32)

    xyz = tf.reshape(xyz, [-1,3])       # align point cloud to center of ground plane
    rgb = tf.reshape(rgb, [-1,3])
    face = tf.reshape(face, [-1,3])
    label = tf.reshape(label, [-1])

    assert((tf.reduce_min(rgb)>=0) & (tf.reduce_max(rgb)<=1))
    if is_training:  # augment vertex
        xyz, rgb = augment_fn(xyz, rgb)
        vertex = tf.concat([xyz, rgb], axis=-1)

        # random drop out facets with probability 0.5
        vertex, face, label, \
        face_mask = Augment.random_drop_vertex(vertex, face,
                                               vertex_label=label,
                                               drop_rate=0.15, prob=0.5)
        xyz, rgb = vertex[:,:3], vertex[:,3:]

    assert((tf.reduce_min(rgb)>=0) & (tf.reduce_max(rgb)<=1))

    vertex = xyz
    face_texture = tf.concat([tf.gather(rgb,face[:,0]),
                              tf.gather(rgb,face[:,1]),
                              tf.gather(rgb,face[:,2])],axis=-1)
...
class MyModel(tf.Module):
    ...
    def test_step(self, meshes, seg_labels):
        ...
        pred_logits += self.model(vertex_in, face_in, nv_in, mf_in,
                                 face_texture, bary_coeff, num_texture, False, True)
        indices = tf.where((seg_labels>=0) & (seg_labels<NUM_CLASSES))  # the considered indices
        pred_logits = tf.gather_nd(pred_logits, indices)
        seg_labels = tf.gather_nd(seg_labels, indices)
        onehot_labels = tf.one_hot(seg_labels, depth=NUM_CLASSES)
