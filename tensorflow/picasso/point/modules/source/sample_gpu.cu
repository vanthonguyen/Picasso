...
__global__ void farthestpointsample3DKernel(int b,int n,int m,const float * __restrict__ dataset,float * __restrict__ temp,int * __restrict__ idxs){
  if (m<=0)
    return;
  const int BlockSize=1024;
  __shared__ float dists[BlockSize];
...
{
  farthestpointsample3DKernel<<<b,1024>>>(b,n,m,inp,temp,out);
}
