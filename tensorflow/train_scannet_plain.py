...
def parse_fn(item, is_training=True):
    ...
    dense_label = tf.io.decode_raw(features['seg_label_raw'], tf.int32)
    ...
    assert((tf.reduce_min(dense_rgb)>=0) & (tf.reduce_max(dense_rgb)<=1))
    if is_training:  # augment vertex
        dense_xyz, dense_rgb = augment_fn(dense_xyz, dense_rgb)
        dense_vertex = tf.concat([dense_xyz, dense_rgb], axis=-1)

        # random drop out vertices with probability 0.5
        dense_vertex, dense_face, dense_label, \
        face_mask = Augment.random_drop_vertex(dense_vertex, dense_face,
                                               vertex_label=dense_label,
                                               drop_rate=0.15, prob=0.5)
        dense_xyz, dense_rgb = dense_vertex[:,:3], dense_vertex[:,3:]

    # dense_rgb -= 0.5
    # assert((tf.reduce_min(dense_rgb)>=-0.5) & (tf.reduce_max(dense_rgb)<=0.5))
    assert((tf.reduce_min(dense_rgb)>=0) & (tf.reduce_max(dense_rgb)<=1))

    # mesh voxelization on-the-fly
    dense_vertex = tf.concat([dense_xyz, dense_rgb], axis=-1)
    vertex, face, \
    label = meshUtil.voxelize_mesh(dense_vertex, face=dense_face,
                                   voxel_size=opt.voxel_size, seg_labels=dense_label)
    ...
    # print(texture.shape, bary_coeff.shape, tf.reduce_sum(num_interior))
    # temp_face_indices = tf.repeat(tf.range(dense_face.shape[0]), num_interior)
    # temp_face = tf.gather(dense_face, temp_face_indices)
    # inpoints = bary_coeff[:,0:1]*tf.gather(dense_vertex,temp_face[:,0]) +\
    #            bary_coeff[:,1:2]*tf.gather(dense_vertex,temp_face[:,1]) +\
    #            bary_coeff[:,2:]*tf.gather(dense_vertex,temp_face[:,2])
    # mesh = o3d.geometry.TriangleMesh()
    # mesh.vertices = o3d.utility.Vector3dVector(dense_vertex.numpy())
    # mesh.triangles = o3d.utility.Vector3iVector(dense_face.numpy())
    # o3d.visualization.draw_geometries([mesh], mesh_show_back_face=True)
    # cloud = o3d.geometry.PointCloud()
    # cloud.points = o3d.utility.Vector3dVector(inpoints.numpy())
    # cloud.colors = o3d.utility.Vector3dVector(texture.numpy())
...
class MyModel(tf.Module):
    ...
    def train_step(self, meshes, seg_labels):
        ...
        with tf.GradientTape() as tape:
            ...
            indices = tf.where((seg_labels>=0) & (seg_labels<NUM_CLASSES))  # the considered indices
    ...
    def evaluate_iou(self, gt_labels, pred_labels):
...
if __name__=='__main__':
    ...
    model.fit(train=trainSet, test=valSet, epochs=300, manager=manager)
